...
static float small_gimbal_angle_deg[3] = {0.0f, 0.0f, 0.0f};
static float big_gimbal_angle_deg[3] = {0.0f, 0.0f, 0.0f};
...
//		return 1; 
	...
	switch(cmd_id)
	{
		case MSG_IMU_INFO_ID:
			upc_cmd_imu_handler(&rx_data[UPC_HEADER_LEN+2]);
			break;
...
void upc_cmd_imu_handler(uint8_t* data)
{
	...
	uint8_t value4 = data[12];
	unpack_4bytes_to_floats(&data[0], &value1);
	unpack_4bytes_to_floats(&data[4], &value2);
	unpack_4bytes_to_floats(&data[8], &value3);
	// eular_from_upc[0] = value1;
	// eular_from_upc[1] = value2;
	// eular_from_upc[2] = value3;
}
...
void upc_cmd_shoot_handler(uint8_t* data)
{
	...
	send_data[0] = data[12];
	unpack_4bytes_to_floats(&data[0], &value1);
	pack_float_to_4bytes(value1, &send_data[4]);
...
void comm_cmd_big_gimbal_imu_handler(uint8_t* data)
{
	unpack_4bytes_to_floats(&data[0], &big_gimbal_angle_deg[0]);
	unpack_4bytes_to_floats(&data[4], &big_gimbal_angle_deg[1]);
	unpack_4bytes_to_floats(&data[8], &big_gimbal_angle_deg[2]);
}
