...
static chassis_ctrl_t* chassis_ctrl_ptr;
...
void ctrl_data_update(rc_ctrl_t *rc_ctrl_ptr, upc_t *upc_ptr)
{
    if(Online_Monitors(rc_ctrl_ptr->last_online_time, RC_ONLINE))
        chassis_ctrl_ptr->ctrl = rc_ctrl_ptr->s1;
    else
        chassis_ctrl_ptr->ctrl = CHASSIS_RC_OFFLINE;
    ...
    if(chassis_ctrl_ptr->ctrl == CHASSIS_RC)
    {
        chassis_ctrl_ptr->mode = 1;//rc_ctrl_ptr->s2;
        upc_ptr->start_upc_flag = 0;
        vx = (fp32)rc_ctrl_ptr->ch3;
        vy = (fp32)rc_ctrl_ptr->ch2;
        vw = (fp32)(rc_ctrl_ptr->ch1);

        chassis_ctrl_ptr->gimbal_shutdown_flag = 0;
        yaw_delta = (fp32)(rc_ctrl_ptr->ch0 * GIMBAL_ANGLE_DELTA_MAX / 660.0f);
        chassis_ctrl_ptr->given_gimbal_yaw += yaw_delta;
    }
    else if(chassis_ctrl_ptr->ctrl == CHASSIS_UPC)
    {
				upc_ptr->start_upc_flag = 1;
        if(Online_Monitors(upc_ptr->last_online, UPC_ONLINE))
        {
            chassis_ctrl_ptr->mode = 1;//upc_ptr->mode;
            ...
            chassis_ctrl_ptr->gimbal_shutdown_flag = 0;
            chassis_ctrl_ptr->given_gimbal_yaw = upc_ptr->gimbal_yaw;
        }
        else
        {
            chassis_ctrl_ptr->mode = 0;
            vx = 0.0f;
            vy = 0.0f;
            vw = 0.0f;
    ...
    chassis_ctrl_ptr->given_chassis_v[0] = norm_v * CHASSIS_MAX_V;
    chassis_ctrl_ptr->given_chassis_v[1] = atan2(vy_filter, vx_filter);
    chassis_ctrl_ptr->given_chassis_w = vw_filter * CHASSIS_MAX_W;
}

void chassis_ctrl_init(void)
{
    chassis_ctrl_ptr = get_chassis_ctrl_data();

    chassis_ctrl_ptr->given_chassis_v[0] = 0.0f;
    chassis_ctrl_ptr->given_chassis_v[1] = 0.0f;
    chassis_ctrl_ptr->given_chassis_w = 0.0f;
    chassis_ctrl_ptr->ctrl = CHASSIS_RC_OFFLINE;
    chassis_ctrl_ptr->mode = 1;
    chassis_ctrl_ptr->gimbal_shutdown_flag = 1;
    chassis_ctrl_ptr->last_gimbal_shutdown_flag = 1;
